...
void Game::getResourceCards(int minPlayers) {
	...
	int cost = 0;
	int age= 1;
	firstAgeDeck.push_back(std::unique_ptr<ResourceCard>(new ResourceCard("lumberYard", age, cost, 1, 0, 0, 0, minPlayers)));
	firstAgeDeck.push_back(std::unique_ptr<ResourceCard>(new ResourceCard("stonePit", age, cost, 0, 1, 0, 0, minPlayers)));
	firstAgeDeck.push_back(std::unique_ptr<ResourceCard>(new ResourceCard("clayPool", age, cost, 0, 0, 1, 0, minPlayers)));
	firstAgeDeck.push_back(std::unique_ptr<ResourceCard>(new ResourceCard("oreVein", age, cost, 0, 0, 0, 1, minPlayers)));

	cost = 1;
	age = 2;

	secondAgeDeck.push_back(std::unique_ptr<ResourceCard>(new ResourceCard("sawmill", age, cost, 2, 0, 0, 0, minPlayers)));
	secondAgeDeck.push_back(std::unique_ptr<ResourceCard>(new ResourceCard("quarry", age, cost, 0, 2, 0, 0, minPlayers)));
	secondAgeDeck.push_back(std::unique_ptr<ResourceCard>(new ResourceCard("brickyard", age, cost, 0, 0, 2, 0, minPlayers)));
	secondAgeDeck.push_back(std::unique_ptr<ResourceCard>(new ResourceCard("foundry", age, cost, 0, 0, 0, 2, minPlayers)));

	int either_or = 1;
	age = 1;

	firstAgeDeck.push_back(std::unique_ptr<ResourceCard>(new ResourceCard("clayPit", age, cost, 0, 0, 1, 1, either_or, minPlayers)));
	firstAgeDeck.push_back(std::unique_ptr<ResourceCard>(new ResourceCard("timberYard", age, cost, 1, 1, 0, 0, either_or, minPlayers)));	

	if (minPlayers > 3) {
		firstAgeDeck.push_back(std::unique_ptr<ResourceCard>(new ResourceCard("excavation", age, cost, 0, 1, 0, 1, either_or, minPlayers)));
	}

	if (minPlayers > 4) {
		firstAgeDeck.push_back(std::unique_ptr<ResourceCard>(new ResourceCard("forestCave", age, cost, 1, 0, 0, 1, either_or, minPlayers)));
	}

	if (minPlayers > 5) {
		firstAgeDeck.push_back(std::unique_ptr<ResourceCard>(new ResourceCard("mine", age, cost, 0, 1, 0, 1, either_or, minPlayers)));
		firstAgeDeck.push_back(std::unique_ptr<ResourceCard>(new ResourceCard("treeFarm", age, cost, 1, 0, 1, 0, either_or, minPlayers)));
	}
}
...
void Game::getBlueCards(int minPlayers) {
	...
	firstAgeDeck.push_back(std::unique_ptr<BlueCard>(new BlueCard("altar", age, points, minPlayers)));
	firstAgeDeck.push_back(std::unique_ptr<BlueCard>(new BlueCard("theater", age, points, minPlayers)));

	points = 3;
	firstAgeDeck.push_back(std::unique_ptr<BlueCard>(new BlueCard("baths", 0, 1, 0, 0, age, points, minPlayers)));
	firstAgeDeck.push_back(std::unique_ptr<BlueCard>(new BlueCard("temple", 0, 1, 0, 0, 1, 0, 0, age, points, minPlayers)));

	age = 2;
	points = 4;
	secondAgeDeck.push_back(std::unique_ptr<BlueCard>(new BlueCard("statue", 1, 0, 0, 2, age, points, minPlayers)));
	secondAgeDeck.push_back(std::unique_ptr<BlueCard>(new BlueCard("courthouse", 0, 0, 2, 0, 0, 1, 0, age, points, minPlayers)));

	points = 5;
	secondAgeDeck.push_back(std::unique_ptr<BlueCard>(new BlueCard("aqueduct", 0, 3, 0, 0, age, points, minPlayers)));
	secondAgeDeck.push_back(std::unique_ptr<BlueCard>(new BlueCard("gardens", 1, 0, 2, 0, age, points, minPlayers)));

	age = 3;
	points = 6;
	thirdAgeDeck.push_back(std::unique_ptr<BlueCard>(new BlueCard("townHall", 0, 2, 0, 1, 1, 9, 9, age, points, minPlayers)));
	thirdAgeDeck.push_back(std::unique_ptr<BlueCard>(new BlueCard("senate", 2, 1, 0, 1, age, points, minPlayers)));

	points = 7;
	thirdAgeDeck.push_back(std::unique_ptr<BlueCard>(new BlueCard("pantheon", 0, 0, 2, 1, 1, 1, 1, age, points, minPlayers)));

	points = 8;
	thirdAgeDeck.push_back(std::unique_ptr<BlueCard>(new BlueCard("palace", 1, 1, 1, 1, 1, 1, 1, age, points, minPlayers)));

	if (minPlayers > 3)
		thirdAgeDeck.push_back(std::unique_ptr<BlueCard>(new BlueCard("pawnshop", 0, 1, 0, 0, age, points, minPlayers)));
}
